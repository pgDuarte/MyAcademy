...
if (strcmp($_POST['type'], "insertexam")==0) {
  ...
  //preparar queries
    ...
    //insert theexam
    $query_theexam->bindValue(":inst",$inst, PDO::PARAM_STR);
    $query_theexam->bindValue(":date",$date != "" ? $date:$ansi, PDO::PARAM_STR);   
    $query_theexam->bindValue(":ansi",$ansi, PDO::PARAM_STR);
    $query_theexam->bindValue(":type",$type, PDO::PARAM_STR);
    $query_theexam->bindValue(":iddocument",$iddocument != 0 ? $iddocument:NULL, PDO::PARAM_STR);
    $query_theexam->bindValue(":idusers",$iduser, PDO::PARAM_STR);
    $query_theexam->execute();
    $idtheexam = $dbh->lastInsertId();
    ...
    if($keywords != "")
    {
        ...
        while($i<$count)
        {
            $dbh->query("INSERT INTO examkeyword (idtheexam, idkeywords) VALUES ('".$idtheexam."',".$keywords[$i].")");
            $i++;
        }
    }
